...
impl Square {
    fn from(input: &str) -> Square {
        // println!("from {} {}", input, input.chars().count());
    ...
    for iteration in 0..iterations {
        ...
        for y in (0..d).rev() {
            ...
            for x in (0..d).rev() {
                ...
                println!("Iteration {}, rule {:?}", iteration, rule);
...
pub fn main() {
    let mut input_file = File::open("input_data.txt").expect("file not found");

    let mut input = String::new();
    input_file.read_to_string(&mut input)
        .expect("something went wrong reading the file");

    let s1 = solution1(&input, 4);
