...
def print_coords(coords_1, coords_2):
    for y in range(-2, 12) if USE_TEST_DATA else range(-2, 200):
        rocks = ""
        for x in range(490, 512) if USE_TEST_DATA else range(450, 520):
            if (x, y) == (500, 0):
                rocks += '+'
...
def get_sand_rest_place(rock_coords, sand_coords, sand_position, maximum_y_value, part):
    ...
    if part == 1:
        if sand_y > maximum_y_value: # TODO
            return 0
    if part == 2:
        if sand_y + 1 == maximum_y_value:
            return sand_position

    obstacle_set = rock_coords.union(sand_coords)

    # If position below is free, go to that and iterate
    if not (sand_x, sand_y+1) in obstacle_set:
        return get_sand_rest_place(rock_coords, sand_coords, (sand_x, sand_y+1), maximum_y_value, part)

    # If below and left are free, go to that and iterate
    if not (sand_x-1, sand_y+1) in obstacle_set:
        return get_sand_rest_place(rock_coords, sand_coords, (sand_x-1, sand_y+1), maximum_y_value, part)
...
def part_1(part):
    ...
    maximum_y_value += 2 if part == 2 else 0

    rock_coords = set()
    for rock in rock_corners:
        rock_coords = add_rock_to_rock_set(rock, rock_coords)
    
    print('max', maximum_y_value)

    sand_coords = set()
    while True:
        new_sand_pos = get_sand_rest_place(rock_coords, sand_coords, START_TUPLE, maximum_y_value, part)
        if new_sand_pos == 0:
