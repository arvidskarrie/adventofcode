...
class Dijkstra:
    ...
    def __init__(self, input):
        ...
        # self.all_nodes_dict = {
        #         [Node(line, col, int(input[line][col]))
        #         for col in range(self.side_length) if not (line == 0 and col == 0)
        #         ]
    ...
    def print_distances(self):
        for i in range(self.side_length):
            dist_line = [
                (self.all_nodes[i][j].distance, self.all_nodes[i][j].limitations)
    ...
    def run(self, input):
        ...
        while True:
            ...
            # If it exists other nodes with the same coordinates with stricter restrictions, delete them.
            # TODO
            ...
            for (n_line, n_col, dir) in neighbour_list:
                ...
                new_limitation[dir] = curr_lim[dir] + 1
                if new_limitation[dir] > 3:
                    continue
                new_limitation = tuple(new_limitation)

                # Not allowed to move backwards
                if (dir == UP and curr_lim[DOWN] != 0) or \
                   (dir == RIGHT and curr_lim[LEFT] != 0) or \
                   (dir == LEFT and curr_lim[RIGHT] != 0) or \
                   (dir == DOWN and curr_lim[UP] != 0):
                   continue
