...
def let_bricks_fall(bricks, early_exit = False):
    changes_made = False
...
def part_1():
    ...
    while let_bricks_fall(bricks):
        pass
    
    # Try to remove every brick and see if anything reacts
    # If nothing reacts, it is desintegrationable
    integrationable_bricks = 0
    for brick_idx, brick in enumerate(bricks):
        print("\tIdx {} / {}".format(brick_idx, len(bricks)))
        test_bricks = bricks[:]
        test_bricks.remove(brick)
        if not let_bricks_fall(test_bricks, True):
            integrationable_bricks += 1
            print(integrationable_bricks)
